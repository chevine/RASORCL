...
GRANT CREATE ANY DIRECTORY, ADVISOR TO RAS 
;
...
CREATE OR REPLACE DIRECTORY data_file_dir AS 'D:\Projects\oracle\hr\db-sample-schemas/sales_history/';

CREATE OR REPLACE DIRECTORY data_pump_dir AS 'c:\Oracle\19c\admin\rasdev\dpdump/64F7C1BC1536448092E6D4C5E30EDE04';
...
BEGIN
    SELECT
        trunc(dbms_random.value(1200, 3500), - 2) * 12 * household_count
    INTO income
    FROM
        dual;
...
PCTFREE 10 PCTUSED 40 TABLESPACE ras_users LOGGING
...
CREATE INDEX ras.ras_audit_violation_id_year_propid_idx ON
    ...
    ASC,
        property_id
    ASC )
        TABLESPACE ras_users PCTFREE 10
            STORAGE ( INITIAL 65536 NEXT 1048576 PCTINCREASE 0 MINEXTENTS 1 MAXEXTENTS 2147483645 FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL
            DEFAULT )
        LOGGING;
...
CREATE TABLE ras.ras_property (
    ...
    rental_income_received     NUMBER(*, 0),
...
CREATE OR REPLACE VIEW RAS.LANDLORD_PROPERTIES_BY_CITY_VW ( STATE, COUNTY, MUNICIPALITY, TAX_YEAR, FIRST_NAME, LAST_NAME, TAXID, LANDLORD_ADDRESS, LANDLORD_CITY, LANDLORD_STATE, AUDIT_VIOLATIONS, PROPERTY_LOCATION, PROPERTY_CITY, PROPERTY_STATE, PROPERTY_ID, RENTAL_INCOME_RECEIVED ) AS
SELECT rm.STATE
    , rm.COUNTY
    , rm.MUNICIPALITY
    , rp.TAX_YEAR
...
CREATE OR REPLACE VIEW RAS.SUMMARIZE_PROPERTIES_WITH_TENANTS_BY_PROPERTY_VW ( STATE, COUNTY, MUNICIPALITY, TAX_YEAR, TENANT_GROSS_INCOME, TOTAL_TENANT_RENTAL_PAYMENTS, PROPERTY_ID, PROPERTY_DESCRIPTION, PROPERTY_LOCATION, PROPERTY_CITY, PROPERTY_COUNTRY ) AS
SELECT psum.STATE
    , psum.COUNTY
    , psum.MUNICIPALITY
    , psum.TAX_YEAR
...
CREATE OR REPLACE VIEW RAS.PROPERTY_INFO_VW ( TAX_YEAR, PROPERTY_STATE, PROPERTY_COUNTY, PROPERTY_MUNICIPALITY, PROPERTY_LOCATION, PROPERTY_DESCRIPTION, BLOCK, LOT, LANDLORD_TAX_ID, PERCENTAGE_OWNED, RENTAL_INCOME_RECEIVED, TOTAL_TENANT_RENTAL_PAYMENTS, RENTAL_INCOME_DIFFERENCE, TOTAL_DEDUCTIONS, PROPERTY_TAXES, OWNER_NAME, OWNER_CITY, OWNER_ADDRESS, OWNER_ZIPCODE, OWNER_STATE, OWNER_COUNTRY, PROPERTY_ACCOUNT_NUMBER, YEAR_CONSTRUCTED, DEED_BOOK, DEED_PAGE, DEED_DATE, LONGITUDE, LATITUDE, PROPERTY_ID ) AS
SELECT rpby.TAX_YEAR
...
CREATE OR REPLACE VIEW RAS.SUMMARIZE_RENTAL_PROPERTIES_BY_COUNTY ( STATE, COUNTY, TAX_YEAR, PROPERTY_COUNT, RENTAL_INCOME_RECEIVED, HOUSEHOLD_COUNT, TOTAL_RENTAL_PAYMENTS, RENTAL_INCOME_DIFFERENCE, NUMBER_OF_PROPERTY_OWNERS, TOTAL_DEDUCTIONS, PROPERTY_TAXES, MAP_COUNTY ) AS
SELECT ll.STATE 
    , ll.COUNTY
    , ll.TAX_YEAR
    , ll.PROPERTY_COUNT
...
CREATE OR REPLACE VIEW RAS.SUMMARIZE_LANDLORDS_BY_STATE_VW ( STATE, TAX_YEAR, LANDLORD_COUNT, HOUSEHOLD_COUNT, RENTAL_INCOME_RECEIVED, TOTAL_RENTAL_PAYMENTS, RENTAL_INCOME_DIFFERENCE, TOTAL_DEDUCTIONS, PROPERTY_TAXES ) AS
SELECT srpbc.STATE
    , srpbc.TAX_YEAR
...
CREATE OR REPLACE VIEW RAS.SUMMARIZE_PROPERTIES_BY_CITY_VW ( TAX_YEAR, PROPERTY_STATE, PROPERTY_COUNTY, PROPERTY_MUNICIPALITY, PROPERTY_DESCRIPTION, RENTAL_INCOME_RECEIVED, TOTAL_TENANT_RENTAL_PAYMENTS, RENTAL_INCOME_DIFFERENCE, PROPERTY_COUNT ) AS
...
CREATE OR REPLACE VIEW RAS.SUMMARIZE_PROPERTIES_BY_COUNTY_VW ( TAX_YEAR, PROPERTY_STATE, PROPERTY_COUNTY, PROPERTY_DESCRIPTION, RENTAL_INCOME_RECEIVED, TOTAL_TENANT_RENTAL_PAYMENTS, RENTAL_INCOME_DIFFERENCE, PROPERTY_COUNT ) AS
SELECT rpivw.TAX_YEAR
    , rpivw.PROPERTY_STATE
    , rpivw.PROPERTY_COUNTY
    , rpivw.PROPERTY_DESCRIPTION
    ...
    , COUNT(1) AS PROPERTY_COUNT
    FROM RAS.PROPERTY_INFO_VW rpivw
    GROUP BY rpivw.TAX_YEAR, rpivw.PROPERTY_STATE, rpivw.PROPERTY_COUNTY, rpivw.PROPERTY_DESCRIPTION 
;

CREATE OR REPLACE VIEW RAS.SUMMARIZE_PROPERTIES_BY_STATE_VW ( TAX_YEAR, PROPERTY_STATE, PROPERTY_DESCRIPTION, RENTAL_INCOME_RECEIVED, TOTAL_TENANT_RENTAL_PAYMENTS, RENTAL_INCOME_DIFFERENCE, PROPERTY_COUNT ) AS
SELECT rpivw.TAX_YEAR
    , rpivw.PROPERTY_STATE
    , rpivw.PROPERTY_DESCRIPTION
    , SUM(rpivw.RENTAL_INCOME_RECEIVED) AS RENTAL_INCOME_RECEIVED
...
CREATE OR REPLACE VIEW RAS.SUMMARIZE_RENTAL_PROPERTIES_BY_CITY_VW ( STATE, COUNTY, MUNICIPALITY, TAX_YEAR, PROPERTY_COUNT, RENTAL_INCOME_RECEIVED, TOTAL_RENTAL_PAYMENTS, RENTAL_INCOME_DIFFERENCE, NUMBER_OF_PROPERTY_OWNERS, TOTAL_DEDUCTIONS, PROPERTY_TAXES ) AS
SELECT rpc.STATE
    , rpc.COUNTY
    , rpc.MUNICIPALITY
    , rpc.TAX_YEAR
...
CREATE OR REPLACE VIEW RAS.SUMMARIZE_RENTAL_PROPERTIES_BY_COUNTY_VW ( STATE, COUNTY, TAX_YEAR, PROPERTY_COUNT, RENTAL_INCOME_RECEIVED, HOUSEHOLD_COUNT, TOTAL_RENTAL_PAYMENTS, RENTAL_INCOME_DIFFERENCE, NUMBER_OF_PROPERTY_OWNERS, TOTAL_DEDUCTIONS, PROPERTY_TAXES, MAP_COUNTY ) AS
SELECT ll.STATE 
    , ll.COUNTY
    , ll.TAX_YEAR
    , ll.PROPERTY_COUNT
    ...
    , ll.TOTAL_DEDUCTIONS
    , ll.PROPERTY_TAXES
    , CAST(CONCAT(ll.STATE, CONCAT('_', TRANSLATE(REPLACE(UPPER(TRIM(ll.COUNTY)), ' COUNTY', ''), ' ', '_'))) AS VARCHAR(50)) AS MAP_COUNTY
    FROM (
        SELECT srpbc.STATE 
        , srpbc.COUNTY
        , srpbc.TAX_YEAR
        ...
        , SUM(srpbc.TOTAL_DEDUCTIONS) AS TOTAL_DEDUCTIONS
        , SUM(srpbc.PROPERTY_TAXES) AS PROPERTY_TAXES
        FROM RAS.SUMMARIZE_RENTAL_PROPERTIES_BY_CITY_VW srpbc
        GROUP BY srpbc.STATE, srpbc.COUNTY, srpbc.TAX_YEAR
    ) ll
    JOIN (
        SELECT stbc.STATE
...
CREATE OR REPLACE VIEW RAS.SUMMARIZE_RENTAL_PROPERTIES_BY_STATE_VW ( STATE, TAX_YEAR, PROPERTY_COUNT, RENTAL_INCOME_RECEIVED, TOTAL_RENTAL_PAYMENTS, RENTAL_INCOME_DIFFERENCE, NUMBER_OF_PROPERTY_OWNERS, TOTAL_DEDUCTIONS, PROPERTY_TAXES ) AS
SELECT st.STATE 
    , st.TAX_YEAR
    , st.PROPERTY_COUNT
    , st.RENTAL_INCOME_RECEIVED
    ...
    FROM (
        ...
        , SUM(srpbc.TOTAL_DEDUCTIONS) AS TOTAL_DEDUCTIONS
        , SUM(srpbc.PROPERTY_TAXES) AS PROPERTY_TAXES
        FROM RAS.SUMMARIZE_RENTAL_PROPERTIES_BY_COUNTY_VW srpbc
        GROUP BY srpbc.STATE, srpbc.TAX_YEAR
    ) st 
;
...
CREATE OR REPLACE VIEW RAS.SUMMARIZE_TENANTS_BY_CITY_VW ( STATE, COUNTY, MUNICIPALITY, TAX_YEAR, HOUSEHOLD_COUNT, GROSS_INCOME, TOTAL_RENTAL_PAYMENTS, RENTAL_INCOME_RECEIVED, RENTAL_INCOME_DIFFERENCE ) AS
SELECT tnt.STATE
    , tnt.COUNTY
    , tnt.MUNICIPALITY
    , tnt.TAX_YEAR
...
CREATE OR REPLACE VIEW RAS.SUMMARIZE_TENANTS_BY_COUNTY_VW ( TAX_YEAR, STATE, COUNTY, HOUSEHOLD_COUNT, TOTAL_RENTAL_PAYMENTS, RENTAL_INCOME_RECEIVED, RENTAL_INCOME_DIFFERENCE ) AS
...
CREATE OR REPLACE VIEW RAS.SUMMARIZE_TENANTS_BY_STATE_VW ( TAX_YEAR, STATE, HOUSEHOLD_COUNT, TOTAL_RENTAL_PAYMENTS, RENTAL_INCOME_RECEIVED, RENTAL_INCOME_DIFFERENCE ) AS
SELECT stbc.TAX_YEAR
    , stbc.STATE
    , SUM(stbc.HOUSEHOLD_COUNT) AS HOUSEHOLD_COUNT
    , SUM(stbc.TOTAL_RENTAL_PAYMENTS) AS TOTAL_RENTAL_PAYMENTS
...
CREATE OR REPLACE VIEW RAS.TENANT_PROFILE_VW ( STATE, COUNTY, MUNICIPALITY, TAX_YEAR, TENANT_TAX_ID, FIRST_NAME, MIDDLE_NAME, LAST_NAME, GROSS_INCOME, PROPERTY_ID, PROPERTY_LOCATION, PROPERTY_CITY, PROPERTY_COUNTRY ) AS
SELECT rm.STATE
    , rm.COUNTY
    , rm.MUNICIPALITY
    , rptt.TAX_YEAR
